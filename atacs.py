...
class Atacs_Student:
    ...
    def auth(self) -> Atilim_Kimlik.login:
        ...
        payload = {"SAMLResponse": samlR, "RelayState": relayS}
    ...
    def report_open_courses(self) -> None:
        ...
        session.headers.update({"Referer": opc_uri})
    ...
    def opened_lessons_database(self) -> None:
        ...
        def compare_courses():
            ...
            try:
                ...
                course_names = df["Ders Adı ( Türkçe )"]
            except KeyError:
            ...
            dep_name = cur_data["curriculum_name"][0]
            ...
            for course, cname, section, teacher, quota in zip(
                open_courses, course_names, sections, teachers, quotas
            ):
                ...
                if course in set(area_courses["lesson_code"]):
                    ...
                    if dep_name in quota:
                        ...
                        print(
                            f"! opened but no quota ! {course} {cname}, {section}, {teacher}"
                        )
    ...
    def save_term_notes(self, student_number: str) -> None:
        ...
        with open("my_course_notes.html", "a+", encoding="utf-8") as f:
            for term in term_names:
                payload = {"OGRENCI": student_number, "Donem": term}
                ...
                if term not in f.read():
                    ...
                    f.write(
                        f"<p style='margin:0;padding-top:0;'>\
                        <b>Term</b>: {term}</p>"
                    )
                    f.write(
                        f"<p style='margin:0;padding-top:0;'>\
